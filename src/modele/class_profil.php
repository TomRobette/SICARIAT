<?php
	class Profil{
		...
		public function __construct($db){
			...
			$this->selectById = $this->db->prepare("SELECT P.id, P.pseudo, P.email, P.idRole, P.photo, P.mdp, R.libelle, P.idRole, P.dateInscription, P.nbArticles, P.nbReplies, P.hideEmail FROM profil P, role R WHERE P.idRole=R.id AND P.id=:id");
			$this->selectByPseudo = $this->db->prepare("SELECT P.id, P.pseudo, P.email, P.idRole, P.photo, P.mdp, R.libelle, P.idRole, P.dateInscription, P.nbArticles, P.nbReplies, P.hideEmail FROM profil P, role R WHERE P.idRole=R.id AND P.pseudo=:pseudo");
			...
			$this->modify = $this->db->prepare("UPDATE profil SET pseudo=:pseudo, email=:email, mdp=:mdp, photo=:photo, idRole=:idRole WHERE id=:id");
		}
		...
		public function modify($id, $pseudo, $email, $mdp, $photo, $idRole){
			$r = true;
			$this->modify->execute(array(':id'=>$id, ':pseudo'=>$pseudo, ':email'=>$email, ':mdp'=>$mdp, ':photo'=>$photo, ':idRole'=>$idRole));
